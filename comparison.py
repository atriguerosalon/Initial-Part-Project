...
def plot_comparison_graphs():
  for i in range(len(filter_sizes)):
    if i==0:
      plt.subplot(2,len(filter_sizes),i+1).text(-171, 171, "$\\bar{\\Phi}_{c,res}$", fontsize=16, fontfamily='serif')
      plt.subplot(2,len(filter_sizes),i+1+len(filter_sizes)).text(-171, 171, "$\\bar{\\Phi}_{c,NN}$", fontsize=16, fontfamily='serif')
