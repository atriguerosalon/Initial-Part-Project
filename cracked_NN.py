...
def get_theoretical_vals():
    for filter_size in filter_sizes:
        ...
        print(filter_size)
        ax = plt.gca()
        fig=plt.pcolor(grid_omega/omega_plus_max_B1, grid_nabla_T/nabla_T_plus_max_B1, phi_NN_field/phi_NN_field.max()*0.95, rasterized=True)
        fig.axes.get_yaxis().set_visible(False)
        plt.xlabel('$\\overline{\\omega}_{c_{T}}^*$', fontsize=26)
        plt.ylabel('$| \\nabla \\tilde{c}_{T}|^*$', fontsize=26)
        plt.colorbar()
